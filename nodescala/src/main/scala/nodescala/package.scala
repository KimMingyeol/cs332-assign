...
package object nodescala {
  ...
  implicit class FutureCompanionOps(val f: Future.type) extends AnyVal {
    ...
    def all[T](fs: List[Future[T]]): Future[List[T]] = {
      ...
      def allAux(fsAux: List[Future[T]], pAuxOpt: Option[Promise[List[T]]]): Unit = fsAux match {
        case Nil => p.tryFailure(new Exception)
        case List(fAux) => pAuxOpt match {
          case None => {
            fAux.foreach{ case v => p.trySuccess(List(v)) }
            fAux.failed.foreach{ case e => p.tryFailure(e) }
          }
          case Some(pAux) => {
            pAux.future.foreach{
              case acc => {
                fAux.foreach{ case v => p.trySuccess(acc :+ v) }
                fAux.failed.foreach{ case e => p.tryFailure(e) }
              }
            }
          }
        ...
        case fAux::fsAuxTail => {
          val pNextAux = Promise[List[T]]
          pAuxOpt match {
            case None => {
              fAux.foreach{ case v => {
                  allAux(fsAuxTail, Some(pNextAux))
                  pNextAux.success(List(v))
                }
              }
              fAux.failed.foreach{ case e => p.tryFailure(e) }
            }
    ...
    def any[T](fs: List[Future[T]]): Future[T] = {
      ...
      def anyAux(fsAux: List[Future[T]]): Unit = fsAux match {
        case Nil => ()
        case fAux::fsAuxTail => {
          fAux.foreach{ case v => p.trySuccess(v) }
          fAux.failed.foreach{ case e => p.tryFailure(e) }
          anyAux(fsAuxTail)
        }
      }
    ...
    def run()(f: CancellationToken => Future[Unit]): Subscription = {
      val cancellableContext = CancellationTokenSource()
      f(cancellableContext.cancellationToken)
      cancellableContext
    }
  ...
  implicit class FutureOps[T](val f: Future[T]) extends AnyVal {
    ...
    def now: T = {
      try {
        Await.result(f, Duration.apply(0, MILLISECONDS))
      } catch {
    ...
    def continueWith[S](cont: Future[T] => S): Future[S] = {
      val p = Promise[S]
      f.foreach{ case _ => p.success(cont(f)) }
      p.future
    }
    ...
    def continue[S](cont: Try[T] => S): Future[S] = {
      val p = Promise[S]
      f.foreach{ case v => p.success(cont(Try(v))) }
      p.future
    }
