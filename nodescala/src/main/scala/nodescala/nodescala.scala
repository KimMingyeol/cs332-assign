...
trait NodeScala {
  ...
  private def respond(exchange: Exchange, token: CancellationToken, response: Response): Unit = {
    ...
    def respondAux(responseListAux: List[String]): Unit = responseListAux match {
      case Nil => ()
      case res::resTail => {
    ...
    val serverLoopSubscription = Future.run() {
      cToken => Future {
        while(cToken.nonCancelled) {
          ...
          while(cToken.nonCancelled && !requestFuture.isCompleted) {}
          val (request, exchange) = requestFuture.now
          respond(exchange, cToken, handler(request))
        }
      }
    }
